...
while($row = $result->fetch_assoc()){ 
	...
	$sql1 = "SELECT sum(cast(money as UNSIGNED INTEGER)) as moneys FROM `test` WHERE (fromid in (SELECT openid FROM `user` where telephone='$telephone')) and TO_DAYS(NOW()) - TO_DAYS(testtime)=$i and pay_status='TRADE_SUCCESS'";
	if($i==8)
		$sql1 = "SELECT sum(cast(money as UNSIGNED INTEGER)) as moneys FROM `test` WHERE (fromid in (SELECT openid FROM `user` where telephone='$telephone')) and DATE_FORMAT(testtime,'%Y%m')=DATE_FORMAT(CURDATE(),'%Y%m') and pay_status='TRADE_SUCCESS'";
	if($i==9)
		$sql1 = "SELECT sum(cast(money as UNSIGNED INTEGER)) as moneys FROM `test` WHERE (fromid in (SELECT openid FROM `user` where telephone='$telephone')) and PERIOD_DIFF(date_format(now(),'%Y%m'),date_format(testtime,'%Y%m'))=1 and pay_status='TRADE_SUCCESS'";
	if($i==10)
		$sql1 = "SELECT sum(cast(money as UNSIGNED INTEGER)) as moneys FROM `test` WHERE (fromid in (SELECT openid FROM `user` where telephone='$telephone')) and pay_status='TRADE_SUCCESS'";
	$result1 = $mysqli1->query($sql1);  
	while($row1 = $result1->fetch_assoc()){ 
		$hisaccounttoday=$row1['moneys']/100*0.1;
	}
